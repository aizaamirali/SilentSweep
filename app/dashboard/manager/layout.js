...
export default function ManagerLayout({ children }) {
    ...
    const navigation = [
        ...
        { name: 'Team', href: '/dashboard/manager/team', icon: UserGroupIcon },
        { name: 'Tasks', href: '/dashboard/manager/tasks', icon: ClipboardDocumentListIcon },
        { name: 'Reports', href: '/dashboard/manager/reports', icon: ChartBarIcon },
        { name: 'Profile', href: '/dashboard/manager/profile', icon: UserIcon },
    ];
    ...
    if (loading || !currentUser || userRole !== 'manager') {
        return (
            ...
            <header className="fixed top-0 left-0 right-0 h-14 bg-white border-b border-gray-200 z-50 shadow-sm">
                <div className="max-w-screen-xl mx-auto px-4 h-full flex items-center justify-between">
                    <div className="flex items-center">
                        <span className="text-xl font-bold text-[#0a66c2]">EMMS Manager</span>
                    </div>
                            ...
                            <div className="relative">
                                <button className="flex items-center hover:bg-gray-100 p-2 rounded-full">
                                    <div className="h-8 w-8 rounded-full bg-[#e7f3ff] flex items-center justify-center text-[#0a66c2] border border-[#0a66c2]">
                                        <span className="text-sm font-bold">{currentUser?.displayName?.charAt(0) || 'M'}</span>
                                    </div>
                    ...
                    <div className="flex-1 flex flex-col pt-5 pb-4 overflow-y-auto">
                        <div className="flex flex-col items-center pb-6 border-b border-gray-200 px-4">
                            <div className="h-20 w-20 rounded-full bg-[#e7f3ff] flex items-center justify-center text-[#0a66c2] border border-[#0a66c2] mb-3">
                                <span className="text-2xl font-bold">{currentUser?.displayName?.charAt(0) || 'M'}</span>
                            </div>
                            <h2 className="text-lg font-medium text-gray-900">{currentUser?.displayName || 'Manager'}</h2>
