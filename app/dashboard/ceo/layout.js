...
export default function CEOLayout({ children }) {
    ...
    useEffect(() => {
        if (!loading) {
            ...
            // If not CEO, redirect to appropriate dashboard
    ...
    const navigation = [
        { name: 'Dashboard', href: '/dashboard/ceo', icon: HomeIcon },
        { name: 'Analytics', href: '/dashboard/ceo/analytics', icon: ChartBarIcon },
        { name: 'Reports', href: '/dashboard/ceo/reports', icon: DocumentChartBarIcon },
        { name: 'Profile', href: '/dashboard/ceo/profile', icon: ShieldCheckIcon },
    ];
    ...
    if (loading || !currentUser || userRole !== 'ceo') {
        return (
            <div className="flex items-center justify-center min-h-screen bg-[#f3f2ef]">
                <div className="flex flex-col items-center">
                    ...
                    <p className="text-gray-900 font-medium">Loading your executive dashboard...</p>
                </div>
            </div>
        );
    }
            ...
            <header className="fixed top-0 left-0 right-0 h-14 bg-white border-b border-gray-200 z-50 shadow-sm">
                <div className="max-w-screen-xl mx-auto px-4 h-full flex items-center justify-between">
                    <div className="flex items-center">
                        <span className="text-xl font-bold text-[#0a66c2]">EMMS Executive</span>
                    </div>
                        ...
                        <div className="flex items-center">
                            <div className="relative">
                                <button className="flex items-center hover:bg-gray-100 p-2 rounded-full">
                                    <div className="h-8 w-8 rounded-full bg-[#ddf5f2] flex items-center justify-center text-[#057642] border border-[#057642]">
                                        <span className="text-sm font-bold">{currentUser?.displayName?.charAt(0) || 'C'}</span>
                                    </div>
                ...
                <div className="flex flex-col flex-grow bg-white border-r border-gray-200 shadow-sm overflow-y-auto">
                    <div className="flex-1 flex flex-col pt-5 pb-4 overflow-y-auto">
                        <div className="flex flex-col items-center pb-6 border-b border-gray-200 px-4">
                            <div className="h-20 w-20 rounded-full bg-[#ddf5f2] flex items-center justify-center text-[#057642] border border-[#057642] mb-3">
                                <span className="text-2xl font-bold">{currentUser?.displayName?.charAt(0) || 'C'}</span>
                            </div>
                            <h2 className="text-lg font-medium text-gray-900">{currentUser?.displayName || 'CEO'}</h2>
                            <p className="text-sm text-gray-600 mt-1">{currentUser?.email}</p>
                            <span className="mt-2 px-3 py-1 bg-[#ddf5f2] text-[#057642] text-xs font-medium rounded-full border border-[#c0e6c0]">
                                Executive
                            </span>
                        </div>
                                ...
                                return (
                                    <Link
                                        ...
                                        className={`group flex items-center px-4 py-3 text-sm font-medium rounded-md transition-colors ${isActive
                                            ? 'bg-[#ddf5f2] text-[#057642]'
                                            : 'text-gray-700 hover:bg-gray-100'
                                            }`}
                                    >
                                        <item.icon
                                            className={`mr-3 h-5 w-5 flex-shrink-0 ${isActive ? 'text-[#057642]' : 'text-gray-500'
                                                }`}
                                        />
                                        {item.name}
                                    </Link>
